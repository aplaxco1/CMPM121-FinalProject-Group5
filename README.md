...
# Devlog Entry 05 - 12/09/2023

## How We Statsfied Software Requirements [F0 - F3]

### [F0 - F2]

For this particular set of requirements, not much refactoring of previous code structure was needed in order to accomplish them, aside from the altering of how certain strings were handled within the game which was seperate from the past requirements, so the code created for the F0-F2 requirements went under very little change at this stage. The only changes that really occurred here was the moving of some code segemnets to other files, so as to have less of a large bulk of code all contained within the same file. In order to do so, the code for the Cell class created for the contiguous byte array requirement was moved
to a seperate Cell.ts file, and the list of CropOptions created for our games internal DSL was moved to the Crop.ts file so that it was more closely placed to its parent Crop class, rather than taking up space
at the top of the Play.ts file. Beyond the moving of code to seperate files and importing them insto the primary Play.ts scene, we also went through and removed some comments that were outdated and unecessary to this version of the code that went unnoticed prior. 

### Internationalization

In order to go about internationalization for this game project, we had to focus on refacting how our game utuilized strings as data types in order to convey some vital game information to the player on the screen itself. In order to do so, strings that were directly displayed on the screen to the player, rather than being hard coded into the program as strings in English were instead gathered from a data structure of all of the text within the game, and inserted into the game at the particular points in which it needed to display that text on the screen. This included, Phaser text objects placed on the UI bar of the game screen, the text on the buttons added in order to satisfy the mobile play requirement to be discussed further below, and the popup alerts which indicated when the game was saved or when the player moved on to the next objective from the scenarios list implemented in the F2 requirements. In order to get the data for this structure of strings, we created several different JSON files containing each of the language options for our game and their translated text. In the Main Menu of the game, at the bottom of the screen, the player has the option to select one of three languages to display the game in (English, Chinese, or Arabic), which, when selected will set the current language within the game to the associated JSON data from the specific JSON file associated with that language. It will then re-write all of the text on the screen using the newly selected text data to the selected language, and when the player loads into the game, the menu scene will pass this data structure into the play scene so that it can read all the associated text in the selected language and display it at the correct locations in game. We added an additional feature, where, if the player were to exit the browser or reaload the game, it would display all the text in the most recently selected language, rather than the default.

In order to add additional language support, a new JSON file would need to be added with the same format as the other language JSON files, that reffering to having all of the same variable entries (en.json, cn.json, and ar.json), and all the translations for each of those pieces of text would be needed to be added to the game. As it is set up now, this would also mean that in the Menu.ts scene, the player would need to load this new language JSON file and assign it to its own variable, and then also create a new button to select this language. A toLocalString() string would also need to be added and set when this language button was pressed for this associated language in order to properly display the correct numerical values and time format. In the Play.ts scene, a new entry would be added to the save data to translate the time of the save to the correct format as determined by the toLocalString() string passed into the scene from the Menu. With all of that added, the game could properly display another potential language.

### Localization

The three languages that our game currently supports include English, Chinese, and Arabic. One of of group members, our Art Lead specifically, was able to do all of the Chinese translations, while enlisting the help of a friend in order to help create the Arabic translations. As we have these languages implemented currently, there are three buttons at the bottom of the Menu screen, for English, Chinese, and Arabic, and when one of them is selected it will update the game to utilize the translations for that particular language. When an option is selected it will update all of the text on the Menu screen, and the player can then start the game and all of the text will be displayed in that language as well. Within the play scene itself, there is no way to change the language, as the buttons for each language only exist on the Menu screen. So, in this case, the player would return to the main menu, select the text option using the associated HTML button, and they could then continue from their save with the updated language. If the player were to close the game's browser page or reload it, when opening the game again, it will default to utilizing the most recently selected language that the player played the game in.
...
Throughout the process of implementing these particular requirements, we faced a few challenges which caused us to change some of our initial plans and switch up some of our roles. One of the first challenges that we had to deal with, of course, involved the implementation of different languages into our game. While the method of adding all the relevant strings and their associated language translations into seperate JSON files was an effective means of implementing this, we did face a bit of difficulty in actually finding all of the different locations within our game that utilized strings to display text to the player. While we moved all the text displayed at the botton of the game screen to a seperate function rather than keeping it in the create() function of the play scene, which made some of it easier to locate, there were several miscelaneous strings we had to locate and change, like the text displayed for certain system alerts, and for the buttons themselves. Lukily, the team member in charge of implementing each of the buttons created a data structure to iterate through to create each button, which made this particular string change much easier. It was also a bit difficult to figure out how to deal with certain digit translations, as Arabic oftentimes utilizes a different numerical system. Previously however, we had done some research on localizing time structures for the displaying of save files to the user and discoevred JavaScripts' toLocalString() method, which tranlates digit strings into their localized formats, which made this digit translation for display sun and water levels much simplier as well.

In terms of getting the mobile installation setup, we had a few difficulties as well, and ended up changing around some roles in order to get this accomplished. Originally, our Testing Lead took charge of handling the mobile installation setup utilizing Capacitor. However, due to some complications with outdated MacOS software not being able to create applications builds for newer mobile iOS phones, this resposibility ended up falling on a different team member, our Engine Lead, who instead was utilizing a Windows machine and had an Android mobile phone. However, as the Teasting Lead had already taken on the responsibilities of figuring out how to set up the environment for Capacitor and the commands necessary to apply it to our project, it was very simple for the Engine Lead to take up the responsibility and slightly alter the approach to instead create an Android application instead of an iOS one. The only complications that the Engine Lead really had involved the utilizing of Android Studio in building the application for mobile devices and porting it, since the Tools Lead did not have to utilize this program to create an iOS version, but luckily they were able to utilize online guides to help out with the process. In this sense, we had some small transfer of responsibility in our team roles, as the Testing Lead took some resposibilities of the Tools and Engine leads in finding the tools to help in creating the build and setting up the code environment to utilize these tools, though the actual installing of the application did end up eventually transfering back to the Engine Lead, who had a machine and mobile device better suited to manage the task.
